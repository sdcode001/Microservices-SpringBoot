...
import com.sd.accounts.services.clients.fallbacks.CardsFeignClientFallback;
...
/**
 * Feign Client interface for the "cards" microservice.
 *
 * 1. SERVICE DISCOVERY LOOKUP (Finding the Targets)
 *    - The identifier "cards" inside @FeignClient is a logical service name, not a real URL.
 *    - When this method is triggered, Spring Cloud queries your Service Registry (like Eureka).
 *    - The registry returns a list of all currently healthy, active network locations (IPs and Ports) for "cards".
 *
 ...
 *    - It selects one specific instance from the pool and replaces the word "cards" with that concrete host:port.
 *
 ...
 *    - **Target Endpoint:** Once the physical URL is ready, Feign builds the HTTP GET request to "/api/cards", injects the query parameter, and executes the network call.
 *    - **Loans Microservice Note:** Feign clients are strictly bound to one logical service. Because this client is declared for "cards", it cannot directly route to a loans service. To hit a loans endpoint, you must declare a companion interface annotated with `@FeignClient("loans")`.
 */
